unit HLVM.Generics.Collections;
...
{$ENDIF}
  ...
  public type
    ...
    TInternalItemPointer = ^TValue;
  private
  public
    FInternalArray: TItemArray;
    FItems: ^TItemArray;
  private
  public
    FCount: Integer;
  ...
  private
    function GetItemPointedInternal(const Key: TKey): TInternalItemPointer;
  protected
  ...
  public
    ...
    property PointedItems[const Key: TKey]: TInternalItemPointer read GetItemPointedInternal;
...
var
  ...
  i, ByteSize, Len: Integer;
begin
  Len := Length(FItems^);
  if NewCapPow2 = Len then
    Exit
  else if NewCapPow2 < 0 then
    OutOfMemoryError;

  ByteSize := SizeOf(TItem) * Len;

  SetLength(newItems, Len);
  if ByteSize > 0 then
    Move(FItems^[0], newItems[0], ByteSize);

  SetLength(FItems^, NewCapPow2);
  for i := 0 to Length(FItems^) - 1 do
    {oldItems} FItems^[i].HashCode := EMPTY_HASH;
  FGrowThreshold := NewCapPow2 shr 1 + NewCapPow2 shr 2;

  for i := 0 to Length(newItems) - 1 do
    if not (newItems[i].HashCode = EMPTY_HASH) then
      RehashAdd(newItems[i].HashCode, newItems[i].Key, newItems[i].Value);
...
function TDictionary<TKey, TValue>.GetItemPointedInternal(const Key: TKey): TInternalItemPointer;
var
  index: Integer;
begin
  index := GetBucketIndex(Key, Hash(Key));
